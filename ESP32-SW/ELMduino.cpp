...
/*
 ...
 Inputs: -------  * Stream &stream - Pointer to Serial port connected  to ELM327
 Return: -------  * bool - Whether or not the ELM327 was propperly  initialized
*/
bool ELM327::begin(Stream &stream)
{
	elm_port = &stream;
...
float ELM327::tanklevel()   // added into original file: https://github.com/PowerBroker2/ELMduino
{
  if (queryPID(SERVICE_01, FUEL_TANK_LEVEL_INPUT))
    return (findResponse() / 1.0);  // percentage of full in 256 fraction

  return ELM_GENERAL_ERROR;
}

float ELM327::Km_scc()  // added into original file: https://github.com/PowerBroker2/ELMduino
{
  if (queryPID(SERVICE_01, DIST_TRAV_SINCE_CODES_CLEARED))
    return (findResponse() / 1.0);
